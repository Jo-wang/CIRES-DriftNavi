            ...
            dbc.Card(
                dbc.CardBody([
                    ...
                    dbc.Row([
                        html.Div([
                            dbc.Label(
                                "Level of Awareness of Dataset Drift in Datasets and AI Models", className="font-weight-bold"),
                            html.Span(
                                html.I(className="fas fa-question-circle"),
                                id="drift-tooltip-snapshot",
                                style={
                                    "fontSize": "20px",
                                    "color": "#aaa",
                                    "cursor": "pointer",
                        ...
                        dbc.Tooltip(
                            "How would you rate your awareness of driftes in datasets and AI models?",
                            target="drift-tooltip-snapshot",
                            placement="right"
                        ),
                        html.Div(
                            dcc.Dropdown(
                                id="drift-awareness-dropdown",
                                options=[
                                    {"label": level, "value": level} for level in
                                    ["Very Low", "Low",
                        ...
                        html.Div(
                            dbc.Checklist(
                                ...
                                options=[
                                    ...
                                    ["drift Detection", "Data Visualization",
...
def handle_auth(signup_clicks, login_clicks, email, password, confirm_password, user_name, professional_role,
                industry_sector, expertise_level, technical_level, drift_awareness, areas_of_interest):
    ...
    if button_id == "submit-signup-button":
        if not all([email, password, user_name, professional_role, industry_sector, expertise_level, areas_of_interest,
                    technical_level, drift_awareness]):
